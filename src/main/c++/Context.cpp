...
JNIEXPORT jboolean JNICALL
Java_org_zeromq_ZMQ_00024Context_setIPv6 (JNIEnv * env, jobject obj, jint enabled)
{
...
JNIEXPORT jint JNICALL
Java_org_zeromq_ZMQ_00024Context_getIPv6 (JNIEnv *env, jobject obj)
{
#if ZMQ_VERSION >= ZMQ_MAKE_VERSION(4,0,0)
    void *c = get_context (env, obj);
    if (! c)
    return -1;
    return zmq_ctx_get (c, ZMQ_IPV6);
#else
    return -1;
#endif
}
